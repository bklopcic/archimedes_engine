...
class PropManager
{
    constructor(game, collisionGroup, otherCollisionGroups) 
    {
        this.game = game;
    ...
    */
    addProp(name, num=10, collide=false)
    {
        ...
        const group = this.game.add.group(this.sortGroup); //create a new group and add it to the parent sort group
    ...
    */
    getProp(name) 
    {
        if (this.particleGroups.hasOwnProperty(name))
        {
            if (this.particleGroups[name].countDead() == 0)
            { //if we don't have any dead objects to revive, make a new one
    ...
    */
    createProp(groupName) 
    {
        const prop = this.particleGroups[groupName].create(0,0,groupName, 0, false);
